...
if(_skipFilter)exitWith{
	_man setVariable ["SFSM_currentPath", (_path apply {ASLToATL _x})];
};
...
for "_i"from 0 to _lastIndex do {
	...
	if(_canFilter)then{
		...
		if(_distance > 5)then{_newPath pushBackUnique _pos; systemChat str _distance};
	}else{
