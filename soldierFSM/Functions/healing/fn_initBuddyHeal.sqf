...
params[
    ...
    "_excluded"     //    array [object(man)]
];
_incapacitated = _incapacitated select {(!(_x in _excluded)) && {[_healer, _x] call SFSM_fnc_canBuddyHeal;}};
_wounded       = _wounded       select {(!(_x in _excluded)) && {[_healer, _x] call SFSM_fnc_canBuddyHeal;}};
